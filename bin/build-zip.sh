...
# Version: 2.1.1
# Special build which excludes the dist folder from i18n generation as was causing memory overflow
...
if ! php -d memory_limit=512M "$(which wp)" i18n make-pot . "$POT_FILE" --domain="$PLUGINSLUG" --exclude="dist,angular-app/dist,node_modules"; then
